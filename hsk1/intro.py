...
class Sections:
    listening = "hsk1_listening"
    reading = "hsk1_reading"
    words = "hsk1_words"
    full_test = "hsk1_full_test"
...
@router.message(Command("hsk1"))
async def get_sections(msg: Message):
    level_text = "Вы выбрали 1 уровень экзамена HSK"
...
async def get_back_to_types(bot: Bot, chat_id: int, section: str):
    ...
    sections_btn = InlineKeyboardButton(text="РАЗДЕЛЫ", callback_data="back_to_sections_hsk1")
...
@router.callback_query(F.data == "back_to_sections_hsk1")
