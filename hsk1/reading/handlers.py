...
TEXT_TASK_1 = "Определите, соответствуют ли картинка и фраза друг другу или нет:"
TEXT_TASK_2 = "Дано 5 предложений. Определите для каждого соответствующую картинку:"
TEXT_TASK_3 = "Сопоставьте вопросы с ответами:"
TEXT_TASK_4 = "Дано 5 предложений с пропусками слов. Выберите для каждого предложения правильное слово:"
...
async def handle_first_task(bot: Bot, state: FSMContext):
    ...
    if index < len(questions):
        ...
        await state.set_state(HSK1ReadingFirstTask.answer)
...
async def handle_second_task(bot: Bot, state: FSMContext):
    ...
    if index < len(sentences):
        ...
        await state.set_state(HSK1ReadingSecondTask.answer)
    else:
        await state.update_data(
            total_score=data["total_score"] + score
        )
...
@router.poll_answer(HSK1ReadingSecondTask.answer)
...
async def handle_third_task(bot: Bot, state: FSMContext):
    ...
    if index < len(sentences):
        ...
        await state.set_state(HSK1ReadingThirdTask.answer)
    else:
        await state.update_data(
            total_score=data["total_score"] + score
        )
...
@router.poll_answer(HSK1ReadingThirdTask.answer)
...
async def start_part_4(bot: Bot, state: FSMContext):
    ...
    if tasks := await service.get_fourth_tasks_by_variant(variant_id=variant_id):
        task = tasks[0]
...
async def handle_fourth_task(bot: Bot, state: FSMContext):
    ...
    if index < len(sentences):
        ...
        await state.set_state(HSK1ReadingFourthTask.answer)
    else:
        await state.update_data(
            total_score=data["total_score"] + score
        )
...
@router.poll_answer(HSK1ReadingFourthTask.answer)
