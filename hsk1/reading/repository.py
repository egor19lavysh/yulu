...
class ReadingRepository:
    ...
    async def get_reading_variants(self) -> list[ReadingHSK1]:
        """Получает все доступные варианты listening заданий"""
        variants = (await self.db_session.execute(select(ReadingHSK1))).scalars().all()
        return variants

    async def get_reading_variant(self, variant_id: int) -> ReadingHSK1:
    ...
    async def get_first_tasks_by_variant(self, variant_id: int) -> list[ReadingFirstTaskHSK1]:
    ...
    async def get_second_tasks_by_variant(self, variant_id: int) -> list[ReadingSecondTaskHSK1]:
    ...
    async def get_third_tasks_by_variant(self, variant_id: int) -> list[ReadingThirdTaskHSK1]:
    ...
    async def get_fourth_tasks_by_variant(self, variant_id: int) -> list[ReadingFourthTaskHSK1]:
...
async def get_reading_repository() -> ReadingRepository:
