...
class ReadingThirdTaskHSK5Question(Base):
    ...
    text: Mapped[str]
    correct_letter: Mapped[str] = mapped_column(String(1))
    options: Mapped[list["ReadingThirdTaskQuestionOptionHSK5"]] = relationship(
        "ReadingThirdTaskQuestionOptionHSK5", 
        back_populates="question"
    )

    task_id: Mapped[int] = mapped_column(ForeignKey("hsk5_reading_third_tasks.id"))
    task: Mapped["ReadingThirdTaskHSK5"] = relationship(
...
class ReadingThirdTaskQuestionOptionHSK5(Base):
    ...
    question_id: Mapped[int] = mapped_column(ForeignKey("hsk5_reading_third_tasks_questions.id"))  # Fixed table name
    question: Mapped["ReadingThirdTaskHSK5Question"] = relationship(
