...
from hsk3.services import listening_service  # Убедитесь, что сервис корректно получает данные из репозиториев
...
async def show_listening_variants(callback: CallbackQuery):
    ...
    variants = listening_service.get_listening_variants()
    if not variants:
